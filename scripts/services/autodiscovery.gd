...
func scan_lan_servers():
	...
	self.client.set_dest_address("255.255.255.255", Settings.get_option("discovery_port"))
...
func start_autodiscovery_server():
	...
	self.server.listen(Settings.get_option("discovery_port"),'0.0.0.0')
	...
	if self.is_servering:
		...
		if self.server.is_connection_available():
			...
			if peer.get_packet().decode_var(0)['type'] == 'get_server':
				peer.put_var({
					"port": Settings.get_option("game_port"),
					"map": Multiplayer.selected_map,
					"players": str(Multiplayer.players.size()) + "/" + str(Multiplayer.player_limit + 1),
					"joinable": Multiplayer.players.size() < Multiplayer.player_limit + 1
				})
