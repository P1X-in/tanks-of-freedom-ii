...
func place_ground(position, name, rotation):
	...
	if not Settings.get_option("shadows"):
...
func place_frame(position, name, rotation):
	...
	if not Settings.get_option("shadows") or not Settings.get_option("dec_shadows"):
		self._disable_shadow(new_element)
...
func force_place_unit(position, name, rotation, side=null, ai_paused=false):
	...
	if self.enable_health and Settings.get_option("show_health"):
...
func attach_mouse_layer():
	...
	MouseLayer.initialize(self.map.model.SIZE, self.map.TILE_SIZE)
	if MouseLayer.mouse_layer.get_parent():
		MouseLayer.mouse_layer.get_parent().remove_child(MouseLayer.mouse_layer)
	self.map.tiles_ground_anchor.add_child(MouseLayer.mouse_layer)
	for key in MouseLayer.ground_points.keys():
		ground_point = MouseLayer.ground_points[key]
...
func _disable_shadow(tile):
	if tile.shadow_override and Settings.get_option("shadows"):
		return
