...
func lob_at_position(destination_position, tween_time):
    ...
    var mesh_translation = self.mesh.get_translation()
    var peak_altitude = Vector3(0, self.ALTITUDE, 0)
    self.tween2.interpolate_property(mesh, "translation", mesh_translation, peak_altitude, tween_time/2.0, Tween.TRANS_CUBIC, Tween.EASE_OUT)
    self.tween2.start()
    ...
    self.tween3.start()
    yield(self.get_tree().create_timer(tween_time/2.0), "timeout")
    self.tween2.interpolate_property(mesh, "translation", peak_altitude, mesh_translation, tween_time/2.0, Tween.TRANS_CUBIC, Tween.EASE_IN)
    self.tween2.start()
