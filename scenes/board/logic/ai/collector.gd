...
func _gather_all_actions():
    ...
    var buildings_actions = self._gather_building_actions(buildings, enemy_buildings, enemy_units, ap)
    var units_actions = self._gather_unit_actions(units, enemy_buildings, enemy_units, ap)

    return buildings_actions + units_actions

func _gather_building_actions(buildings, enemy_buildings, enemy_units, ap):
    ...
    for building_tile in buildings:
        ...
        if brain == null:
            continue
        buildings_actions += brain.get_actions(building_tile, enemy_buildings, enemy_units, ap)

    return buildings_actions

func _gather_unit_actions(units, enemy_buildings, enemy_units, ap):
    var units_actions = []
    var brain
    ...
    for unit_tile in units:
        ...
        if brain == null:
            continue
        units_actions += brain.get_actions(unit_tile, enemy_buildings, enemy_units, ap)
