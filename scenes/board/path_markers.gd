...
func place_marker(tile_key):
    var tile = self.map.model.tiles[tile_key]
...
func get_rotation_to_tile(source_key, destination_key):                
    var source_tile = self.map.model.tiles[source_key]
    var destination_tile = self.map.model.tiles[destination_key]
...
func rotate_marker(marker, source_key, destination_key):
    ...
    marker.set_rotation(Vector3(0, deg2rad(self.rotations[direction]), 0))
